...
public class ControllerErrorTests {
	...
	public void testNoThreadDeadline() {
		...
		assertEquals(
				"Error at PulseOx_Forwarding_Logic.aadl:93: Thread deadline must either be set with Default_Thread_Deadline (at package level) or with Timing_Properties::Deadline (on individual thread)",
				errorSB.toString().trim());
	}
	...
	public void testNoThreadPeriod() {
		...
		assertEquals(
				"Error at PulseOx_Forwarding_Logic.aadl:93: Thread period must either be set with Default_Thread_Period (at package level) or with Timing_Properties::Period (on individual thread)",
				errorSB.toString().trim());
	}
	...
	public void testNoWCET() {
		...
		assertEquals(
				"Error at PulseOx_Forwarding_Logic.aadl:93: Thread WCET must either be set with Default_Thread_WCET (at package level) or with Timing_Properties::Compute_Execution_Time (on individual thread)",
				errorSB.toString().trim());
	}
	...
	public void testIntegerOverflow() {
		...
		assertEquals(
				"Error at PulseOx_Forwarding_Logic.aadl:93: Property Default_Thread_Period on element CheckSpO2Thread converts to 2.5E9 ms, which cannot be converted to an integer\n"
						+ "Error at PulseOx_Forwarding_Logic.aadl:93: Property Default_Thread_Deadline on element CheckSpO2Thread converts to 2.5E9 ms, which cannot be converted to an integer\n"
						+ "Error at PulseOx_Forwarding_Logic.aadl:93: Thread period must either be set with Default_Thread_Period (at package level) or with Timing_Properties::Period (on individual thread)",
				errorSB.toString().trim());
	}
