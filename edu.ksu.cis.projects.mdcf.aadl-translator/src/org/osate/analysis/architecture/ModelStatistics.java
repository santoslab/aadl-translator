...
//
//
//			if (ci instanceof DeviceImplementation)
//				handleDeviceTypeImpl((DeviceImplementation) ci);
//			else if (ci instanceof SystemImplementation)
//				handleSystemTypeImpl((SystemImplementation) ci);
//			else if (ci instanceof ProcessImplementation)
//				handleProcessImplementation((ProcessImplementation) ci);
//			
//			return DONE;
//		}
//
...
//	private class MyInstanceSwitch extends InstanceSwitch<String> {
...
//	}
	...
	protected final void initSwitches() {
		...
		// We don't want to count instance model objects.
//		instanceSwitch = new MyInstanceSwitch();
	...
	public void countClassifier(EClass cl) {
		if (Aadl2Package.eINSTANCE.getComponentType().isSuperTypeOf(cl)) {
			typeCount++;
		} else if (Aadl2Package.eINSTANCE.getComponentImplementation()
				.isSuperTypeOf(cl)) {
			compImplCount++;
		}
	}
	...
	public String getApplicationResult() {
		if (componentCount > 0) {
			return "Application statistics: " + threadCount + " threads, "
					...
					+ " semantic connections, " + ETEFICount
	...
	public String getExecutionPlatformResult() {
		if (componentCount > 0) {
			return "Execution platform statistics: " + processorCount
					...
					+ " virtual processors, " + memoryCount + " memory units, "
					+ busCount + " buses, " + virtualBusCount
					+ " virtual buses, " + deviceCount + " devices.  ";
		}
		return null;
	}
}
