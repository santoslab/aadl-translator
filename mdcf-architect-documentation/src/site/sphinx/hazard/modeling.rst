...
For example, consider our pulse oximetry forwarding application. We might decide to model the patient and the clinician, as well as the treatment the clinician performs on the patient.

Modeling these components is done with :construct:`abstract` constructs, while the connections are modeled as :construct:`feature` constructs.
